...
import { Button } from "@/components/layout/ui/ButtonAddNew";
...
type UiStatusFilter = "all" | "confirmed" | "pending" | "cancelled" | "refunded";
...
export default function AdminBookingsPage() {
  ...
  const fetchBookingCount = useCallback(async (status?: BookingStatus) => {
    ...
    const data = (await response.json()) as BookingsResponse & { error?: string };
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-zinc-400">Track and manage all ticket bookings</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            <div className="relative min-w-[260px] flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-zinc-800 bg-zinc-950 py-3 pl-12 pr-4 text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-red-500"
              />
            </div>
            ...
            <select
              ...
              className="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3 transition-all hover:border-zinc-700"
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px]">
              <thead className="border-b border-zinc-800 bg-zinc-950">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-medium text-zinc-400">Booking ID</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-zinc-400">Customer</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-zinc-400">Movie & Showtime</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-zinc-400">Seats</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-zinc-400">Amount</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-zinc-400">Status</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-zinc-400">Actions</th>
                </tr>
              </thead>
                ...
                filteredBookings.map((booking) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <span className="font-mono text-sm font-medium">{formatBookingCode(booking.id)}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <p className="text-xs text-zinc-500">{booking.user.phone || "No phone"}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div>
                        <p className="text-sm font-medium">{booking.showtime.movie.title}</p>
                        <p className="text-xs text-zinc-500">{formatShowtime(booking.showtime.startTime)}</p>
                        <p className="text-xs text-zinc-500">{booking.showtime.hall.name}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <p className="font-semibold">{formatMoney(booking.finalAmount)}</p>
                    ...
                    <td className="px-6 py-4">
                      ...
                      >
                        ...
                        {booking.bookingStatus.charAt(0) + booking.bookingStatus.slice(1).toLowerCase()}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => window.open(`/api/admin/booking/${booking.id}`, "_blank")}
